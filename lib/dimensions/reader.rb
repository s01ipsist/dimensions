...
module Dimensions
  class Reader
    ...
    ICO_HEADER    = [0x00, 0x00, 0x01, 0x00]
    CUR_HEADER    = [0x00, 0x00, 0x02, 0x00]
    ...
    def determine_type
      if @size >= 4
        ...
        if match_header(GIF_HEADER, bytes)
          ...
          @type = :bmp
        elsif match_header(ICO_HEADER, bytes)
          @type = :ico
        elsif match_header(CUR_HEADER, bytes)
          @type = :cur
        end
    ...
    def extract_ico_dimensions
      if @size >= 8
        @width, @height = @data.unpack('x6CC').map{ |v| v.zero? ? 256 : v }
        process nil
      end
    end
    alias_method :extract_cur_dimensions, :extract_ico_dimensions
